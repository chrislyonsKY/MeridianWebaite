...
export interface IStorage {
  ...
  // Stories
  getStories(page: number, limit: number, topic?: string): Promise<{ stories: any[], totalPages: number, currentPage: number }>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getStories(page: number = 1, limit: number = 10, topic?: string) {
    const offset = (page - 1) * limit;

    let query = db.select().from(stories);
    let countQuery = db.select({ value: count() }).from(stories);

    if (topic) {
      query = query.where(eq(stories.topic, topic)) as any;
      countQuery = countQuery.where(eq(stories.topic, topic)) as any;
    }

    // Only get published stories
    query = query.where(eq(stories.status, "published")) as any;
    countQuery = countQuery.where(eq(stories.status, "published")) as any;
